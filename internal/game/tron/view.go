...
// seatColors are the trail colors, by seat: red, cyan, green, yellow. All
// four read well on a dark background.
...
// drawn highlighted so it is easy to find among the others.
...
// drawLegend lists the players in their colors: a block for a living player,
...
// drawTrails draws every occupied cell as a block in its owner's color.
func (g *Game) drawTrails(c *render.Canvas) {
	for y := 0; y < g.cfg.Height; y++ {
		for x := 0; x < g.cfg.Width; x++ {
			if owner := g.grid[g.idx(x, y)]; owner != 0 {
...
// drawHeads draws each head over its trail: a crashed head as an x, the
...
func (g *Game) status(viewer int, seated bool, p game.PlayerID) string {
	...
	switch g.phase {
	case phaseWaiting:
		return "Waiting for another player. Use a terminal of at least 70x24."
	case phaseCountdown:
		return fmt.Sprintf("Starting in %d", g.countdownNumber())
	case phasePlaying:
		if me.alive {
			return "Steer with the arrow keys or W A S D"
		}
